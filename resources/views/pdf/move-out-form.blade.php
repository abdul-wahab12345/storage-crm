...
.brand-sub { font-size: 11px; color: rgba(255,255,255,0.82); margin-top: 4px; line-height: 1.4; }
.doc-title { font-size: 20px; font-weight: 700; color: #fff; text-align: right; letter-spacing: -0.3px; }
.doc-sub { font-size: 10.5px; color: rgba(255,255,255,0.7); text-align: right; margin-top: 3px; letter-spacing: 0.5px; text-transform: uppercase; }
...
.info-table td { padding: 7px 8px; font-size: 12.5px; vertical-align: top; border-bottom: 1px solid #f8fafc; }
.info-table td.label { color: #64748b; width: 36%; font-weight: 700; }
.info-table td.value { color: #1e293b; font-weight: 500; }

/* ── Two-column info ── */
.two-info { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
.two-info td { vertical-align: top; width: 50%; padding: 0; }
.col-r { padding-left: 20px; border-left: 1px solid #f1f5f9; }

/* ── Paragraph Terms ── */
.terms-box { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 4px; padding: 16px 20px; margin: 30px 0; font-size: 13px; color: #334155; line-height: 1.7; text-align: justify; }
...
.sig-label { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; color: #94a3b8; margin-bottom: 6px; }
.sig-name { font-size: 13px; font-weight: 700; color: #1e293b; margin-bottom: 45px; }
.sig-line { border-top: 1px solid #cbd5e1; padding-top: 6px; font-size: 10.5px; color: #64748b; }
...
<body>
    ...
    <div class="body">
                ...
                <td class="col-r">
                    <table class="info-table" cellpadding="0" cellspacing="0">
                        <tr>
                            ...
                            <td class="value">{{ $lease->move_in_date ? $lease->move_in_date->format('d M Y') : '—' }}</td>
                        </tr>
                        <tr>
                            <td class="label">Move Out Date</td>
                            <td class="value" style="font-weight:700; color:#EA580C;">{{ $form->move_out_date ? $form->move_out_date->format('d M Y') : '—' }}</td>
                        </tr>
                        <tr>
        ...
        <div class="terms-box">
            ...
            As your storage rental period has ended, you are collecting your goods from {{ $company }} in your own presence and at your own risk. Please inspect all items at the time of collection. Any complaint must be reported before the goods leave the storage facility.
            <br><br>
            Upon collection of your goods, your storage contract is terminated, and {{ $company }} shall not be responsible for any loss, damage, missing items, or claims thereafter. Thank you for your cooperation.
        </div>
                    ...
                    <table cellpadding="0" cellspacing="0" style="margin-top:8px;">
                        <tr>
                            <td style="vertical-align:bottom; padding-right:16px; height: 60px;">
                                @if ($signatureData)
                                    <img src="{{ $signatureData }}" style="max-height:55px; max-width:140px; display:block;">
                                @endif
                            </td>
                            <td style="vertical-align:bottom; height: 60px;">
                                @if ($stampData)
                                    <img src="{{ $stampData }}" style="max-height:75px; max-width:75px; display:block; opacity:0.88; margin-bottom: -10px;">
                                @endif
                            </td>
                        </tr>
                    </table>
                    <div class="sig-line" style="margin-top: 15px;">Signature &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Date: {{ $form->created_at->format('d M Y') }}</div>
                </td>
            </tr>
        </table>
