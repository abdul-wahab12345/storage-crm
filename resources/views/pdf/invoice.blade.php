...
<head>
    ...
    <style>
        * {
        ...
        .totals-row-table td.label {
            color: #64748b;
        }

        .totals-row-table td.amount {
            text-align: right;
            font-weight: 700;
        }
        ...
        <div class="body-pad">
            ...
            <table class="parties-table" cellpadding="0" cellspacing="0">
                <tr>
                    <td style="vertical-align:top; width:50%;">
                        ...
                        <div class="party-detail">{{ $invoice->tenant->email }}</div>@endif
                        @if ($invoice->tenant?->phone)
                        <div class="party-detail">{{ $invoice->tenant->phone }}</div>@endif
                        @if ($invoice->tenant?->address)
                        <div class="party-detail">{{ $invoice->tenant->address }}</div>@endif
                    </td>
                ...
                <table class="period-table" cellpadding="0" cellspacing="0">
                    <tr>
                        <td style="width:50%;">
                            ...
                            <div class="period-value">{{ $invoice->period_start->format('M j') }} —
            ...
            <table class="items-table" cellpadding="0" cellspacing="0">
                ...
                <tbody>
                    ...
                    @if (is_array($invoice->additional_fees) && count($invoice->additional_fees) > 0)
                        @foreach ($invoice->additional_fees as $fee)
                            <tr>
                                <td>
                                    {{ $fee['description'] ?? 'Additional Fee' }}
                                </td>
                                <td class="right">{{ \App\Models\Setting::money($fee['amount'] ?? 0) }}</td>
                            </tr>
                        @endforeach
                    @endif
                    @if ($invoice->late_fee > 0)
                        <tr>
                            ...
                            <td class="right" style="color:#dc2626;">{{ \App\Models\Setting::money($invoice->late_fee) }}
                            </td>
                        </tr>
                    @endif
                </tbody>
                ...
                <div class="totals-inner">
                    <table class="totals-row-table" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
                            ...
                            <td class="amount">{{ \App\Models\Setting::money($subtotal) }}</td>
                        </tr>
                        @if ($invoice->late_fee > 0)
                            <tr>
                                <td class="label">Late Fee</td>
                                <td class="amount" style="color:#dc2626;">
                                    {{ \App\Models\Setting::money($invoice->late_fee) }}</td>
                            </tr>
                        @endif
